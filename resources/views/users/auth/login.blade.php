...
@endsection
@section('main')
    <section class="w-full bg-light p-bottom-50 align-center br-20 p-y-20 g-10 column p-10 backdrop-blur-10 max-w-500 overflow-hidden">
      
        <img src="{{ asset('logo.png?v=1.1') }}" class="w-half" alt="">
        <strong class="desc c-primary c-white font-cinzel-decorative">Welcome Back</strong>
        <span class="text-light bottom-10">Sign In to your Account</span>
         <form onsubmit="PostRequest(event,this,MyFunc.redirect)" action="{{ url('post/login/process') }}" method="POST" class="column w-full g-10">
        ...
        <input type="hidden" value="{{ csrf_token() }}" name="_token" class="input">
            ...
            <input autocomplete="off" readonly  onfocus="this.removeAttribute('readonly')" placeholder="Username or Email Address" name="id" class="w-full input inp required ph-c-silver h-full no-border no-outline bg-transparent br-10" type="text">
        ...
        <div style="height:40px;margin-right:5px !important;" class="h-semi-full perfect-square br-10 bg-dim right-10 column justify-center">
          ...
          <button class="post top-20 c-white bold">Login</button>
         </form>
          <a href="{{ url('register') }}" class="c-green">New User? Register  Now</a>
    </section>
@endsection
@section('js')
    <script class="js">
