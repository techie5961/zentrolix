...
@section('main')
    <section class="column w-full p-10 g-10">
        <form action="{{ url('admins/post/edit/task/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.Posted)" class="column g-5 p-10 bg-light br-10 box-shadow">
            ...
            <div class="cont bg-dim w-full br-10 border-1 border-color-silver h-50">
                <input value="{{ $data->title ?? 'null' }}" placeholder="Enter Task Title" name="title" type="text" class="inp required input w-full h-full bg-transparent br-10 no-border">
            </div>
            <label for="">Task Link</label>
            <div class="cont bg-dim w-full br-10 border-1 border-color-silver h-50">
                <input value="{{ $data->link ?? '' }}" placeholder="Enter Task Link" type="url" name="link" class="inp required input w-full h-full bg-transparent br-10 no-border">
            </div>
            <label for="">Task Instructions</label>
            <div class="cont bg-dim w-full br-10 border-1 border-color-silver h-200">
             <textarea name="description" id="" class="w-full required inp input h-full br-10 p-10 font-primary no-border no-resize bg-transparent">{{ $data->description }}</textarea>  
            </div>
